...
export class Model171B extends DeviceBase {
	...
	getEnergyData(jsonData, jsonData2) {
		...
		const workMode = (jsonData.B64 >> 12) & 0x03;
		const u1Status = (jsonData.B64 >> 21) & 0x01;
		const u2Status = (jsonData.B64 >> 22) & 0x01;
		const modeMap = { 0: '并网', 1: '离网', 2: '整流' };
		this.energyData.B64.value = `工作模式:${modeMap[workMode] || workMode} U1:${u1Status === 0 ? '开机' : '关机'} U2:${u2Status === 0 ? '开机' : '关机'}`;
...
export class EnergyData {
	...
	B24 = { name: 'A相有功功率(kW)', value: "--" };
	B26 = { name: 'A相无功功率(kVar)', value: "--" };
	B28 = { name: 'B相有功功率(kW)', value: "--" };
	B30 = { name: 'B相无功功率(kVar)', value: "--" };
	B32 = { name: 'C相有功功率(kW)', value: "--" };
	B34 = { name: 'C相无功功率(kVar)', value: "--" };
	B36 = { name: '交流频率(Hz)', value: "--" };
	B40 = { name: '模块面板（环境）温度', value: "--" };
	B44 = { name: '总有功功率(kW)', value: "--" };
	B48 = { name: '总无功功率(kVar)', value: "--" };
	B52 = { name: '总视在功率(kVA)', value: "--" };
	B56 = { name: '直流侧电压', value: "--" };
	B58 = { name: '直流侧电流', value: "--" };
	B60 = { name: '额定输出功率(kW)', value: "--" };
