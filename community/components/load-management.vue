        ...
        <view class="device-group">
          ...
          <view class="device-grid">
            <view class="device-item">
              <div class="number">{{ nyzData.dljQuantity || 1 }}</div>
              <div class="text">多联机</div>
            </view>
            <view class="device-item">
              <div class="number">{{ nyzData.mkjQuantity || 2 }}</div>
              <div class="text">模块机</div>
            </view>
            <view class="device-item">
              <div class="number">{{ nyzData.nyzQuantity || 3 }}</div>
              <div class="text">能源站</div>
            </view>
            <view class="device-item">
              <div class="number">{{ nyzData.unknownDeviceQuantity || 4 }}</div>
              <div class="text">未知设备</div>
            </view>
          </view>
        </view>
      </view>
...
export default {
  ...
  methods: {
    ...
    onDeviceClick(device) {
      console.log(device)
      uni.navigateTo({
        url: `/pages/device-detail/device-detail?deviceInfo=${encodeURIComponent(JSON.stringify(device))}`
      });
    },
    goToAccessSetting() {
      uni.navigateTo({ url: '/pages/access-setting/access-setting' });
    },
...
.card { background: #fff; border-radius: 12px; margin-bottom: 16px; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.08); overflow: hidden; }
...
/* 设备统计 */
.device-group { background: #FFFFFF; border-radius: 12px; padding: 16px; margin-bottom: 16px; }
.group-title { font-size: 16px; font-weight: 600; color: #333; margin-bottom: 12px; }
.device-grid {
  display: flex;
